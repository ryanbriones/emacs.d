...
(defvar ini-directory (concat personal-config-dir "/ini"))
...
(setq system-specific-config (concat personal-config-dir system-name ".el"))
(if (file-exists-p system-specific-config) (load system-specific-config))

;;; Now load all the ini-xxx files in the initialization directory

(let ((ini-directory (concat personal-config-dir "/ini"))
      (files (directory-files ini-directory nil "^.*\\.el$")))
  (while (not (null files))
    (ini-load (substring (car files) 0 -3))
    (setq files (cdr files)) ))

;;; load user-specific configurations here
(setq user-specific-config (concat personal-config-dir user-login-name ".el"))
